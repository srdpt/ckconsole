...
var locations = {
  "FirebaseHQ": [45.4375892, 12.3340369],
  "Caltrain":   [45.4375892, 12.3340369]
};
...
var transitFirebaseRef = new Firebase("https://ckdata.firebaseio.com/")

// Create a new GeoFire instance, pulling data from the public transit data
var geoFire = new GeoFire(transitFirebaseRef.child("_geodata"));
...
geoQuery.on("key_entered", function(vehicleId, vehicleLocation) {
  ...
  // console.log("Found element", vehicleId);

  // Look up the vehicle's data in the Transit Open Data Set
  transitFirebaseRef.child("data").child(vehicleId).once("value", function(dataSnapshot) {
    ...
    if (vehicle !== null && vehiclesInQuery[vehicleId] === true) {
      ...
      //   transitFirebaseRef
      //     .child('data')
      //     .child(vehicle.birth_certificate.ckID)
      //     .on('value', function(res) {
      //       var item = res.val();
      //       // console.log("Item", item);
      //       if (hide_groups.indexOf(item.group_list) > -1) {
      //         // console.log("KO", item.group_list);
      //       } else {
      //         // console.log("OK", item.group_list);
      //         if (vehiclesInQuery[vehicleId]) {
      //           vehicle.marker = createVehicleMarker(vehicle, getVehicleColor(vehicle));
      //         }
      //       }
      //     });
...
function createVehicleMarker(vehicle, vehicleColor) {
  var marker = new google.maps.Marker({
    icon: "https://chart.googleapis.com/chart?chst=d_bubble_icon_text_small&chld=" + 'books' + "|bbT|" + vehicle.birth_certificate.ckID + "|" + 'FF6450' + "|eee",
    position: new google.maps.LatLng(vehicle.birth_certificate.lat, vehicle.birth_certificate.lon),
    optimized: true,
    map: map
  });
